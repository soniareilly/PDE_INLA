...
def MatMvMult(A, x, y):
    '''
    Apply some operator A with a method mult to each column of x and store 
    the outputs in the columns of y
    '''
...
def MatMvTranspmult(A, x, y):
    '''
    Apply the transpose of some operator A that has a method transpmult to each
    column of x and store the outputs in the columns of y
    '''
    assert x.nvec() == y.nvec(), "x and y have non-matching number of vectors"
...
def MvDSmatMult(X, A, Y):
    '''
    Matrix-multiply a Multivector X by an array A and store the output in the Multivector Y
    '''
